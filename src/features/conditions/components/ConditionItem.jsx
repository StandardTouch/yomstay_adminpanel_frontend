...
export default function ConditionItem({
  ...
  const displayName = isArabic
    ? condition.ar?.displayName || condition.displayName
    : condition.displayName;
  const description = isArabic
    ? condition.ar?.description || condition.description
      ...
      >
        ...
        <div className="flex items-center gap-4 ml-4">
          ...
          <div className="flex items-center gap-2">
            <Label
              htmlFor={`active-toggle-${condition.id}`}
              className="text-xs whitespace-nowrap"
            >
              Active:
            </Label>
            <Switch
              id={`active-toggle-${condition.id}`}
              checked={condition.isActive ?? false}
              onCheckedChange={(checked) =>
                onToggleActive(condition.id, checked)
              }
