...
export const AmenityItem = ({ amenity, onDelete, onAdd, CheckSheet }) => (
  <div className="relative" >
    <div className=" cursor-pointer flex items-center gap-2 p-2 border rounded-md h-full min-h-18" onClick={onAdd}>
      {amenity.icon && <img src={amenity.icon} alt={amenity.name} className="w-15 h-15 rounded" />}
      <p>{amenity.name}</p>
    </div>
